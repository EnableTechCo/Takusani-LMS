...
| G3 | Signed upload URL, checksum, and resume claims do not match Storage behaviour | Medium | Resumable upload with signed token; intent expiry as the control; size from Storage; server-computed SHA-256 in the scan step | Decided; spike X-5 confirmed 23 Sep 2026 | Data model "Submissions and files"; API |
...
| U-03 | The staging project has Supabase's automatic RLS event trigger (`ensure_rls`, `public.rls_auto_enable()`), which no migration creates. Keep it or remove it? (S1-17) | Remove it everywhere by migration. It only acts on tables in `public`, which holds nothing of ours and is not exposed; the function was security definer and callable by `anon`. `public` is kept empty by a pgTAP test. | Decided 22 September 2026 | ADR-028; migration `20260924090000`; test `0008_public_schema_empty` |
...
| X-5 | 25 MB resumable upload with a signed token on an interrupted connection; record which integrity fields Storage metadata exposes | G3 | Upload handshake | **Confirmed 23 Sep 2026.** 25 MB uploaded straight to the private bucket with the learner's own session over TUS; broken part way, it resumed from the last chunk Storage had stored rather than starting again, and arrived byte for byte. A key no intent authorises is refused by the storage policy itself. Storage metadata exposes `eTag`, `size`, `mimetype`, `cacheControl`, `lastModified`, `contentLength`: no content hash, so the learner's checksum stays declared and the authoritative SHA-256 belongs to the scan step. Kept as a CI check: `scripts/check-resumable-upload.mjs`. |
