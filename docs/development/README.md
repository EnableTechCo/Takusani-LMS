...
- **Tasks** (`src/modules/submissions`): a facilitator or the cohort's coordinator writes a task as a draft, sets its rubric and audience, then publishes it. A draft is invisible to learners; a published task is fixed, because learners plan around it, so a change means a new task. Teaching, then Tasks.
- **Uploads** (`src/modules/submissions`, ADR-007): evidence goes straight from the browser to the private `submissions` bucket. `api.authorise_upload()` records an intent (one random object key, the limits, a two-hour expiry); a storage policy lets the learner write exactly that key while the intent is live; `api.finalise_upload()` reads the real size and type from Storage and records the file. Run `npm run db:uploads` to exercise the whole path, including a 25 MB upload interrupted part way.
- **Submissions** (`src/modules/submissions`, FR-308 to FR-311): a learner hands work in against a published task. Each submission is an immutable version with its own receipt; handing in again adds the next version and keeps the last one. The server decides lateness from the task's due date and late policy, and a task whose required evidence has no file cannot be submitted: the missing requirement is named. A retry with the same attempt identifier returns the first receipt rather than making a second version.
- **The learner's task page** (`/learn/tasks`, `/learn/tasks/[taskId]`, `.../submit`): the brief, what to hand in, how it is marked, and every version with its receipt. The upload is the only part that needs the browser: files go straight to Storage over the resumable protocol, so a dropped connection carries on where it stopped, and a file uploaded but not yet submitted is still there after a reload, against the requirement it answers.
- **Results** (`assessment` schema, ADR-021): one result per learner and assessable item carries the current outcome. Submitting opens an assessment instance on it; a resubmission belongs to the same result. Decisions are append-only and form one chain (no forks, one root). Releasing a held result happens once and writes the release time, the release sequence and the exclusive appeal deadline together; a trigger refuses any other change to them.
- **Marking** (`src/modules/assessment`, `/assess`): an assessor sees the work waiting in the cohorts their role covers, takes an item so nobody else marks it at the same time, reads the evidence through short-lived links, and fills in the rubric, feedback and outcome. The draft saves itself and says when; a save from an older tab is refused rather than overwriting. Opening an item outside your scope is a 404 and is written to the audit log.
- **Finalising** (`api.finalise_decision`, FR-408, BR-04): the assessor's confirmation says what will happen before they commit. In a cohort that is not moderated the result is released at once, with its seven-day appeal window and any resubmission deadline; in a moderated cohort it is held, and the learner sees nothing until moderation is signed off. The decision records the marks, feedback and remediation the learner will read, and cannot be edited afterwards.
