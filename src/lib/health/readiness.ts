 ...
 */
export async function checkReadiness(env: NodeJS.ProcessEnv = process.env, fetchImpl: typeof fetch = fetch): Promise<Readiness> {
  ...
  try {
    const response = await fetchImpl(`${url}/rest/v1/rpc/health_check`, {
      method: "POST",
      headers: { apikey: key, "content-type": "application/json", "content-profile": "api", accept: "application/json" },
      body: "{}",
      cache: "no-store",
