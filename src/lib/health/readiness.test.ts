...
const env = { NEXT_PUBLIC_SUPABASE_URL: "http://127.0.0.1:54321", NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY: "sb_publishable_test" } as unknown as NodeJS.ProcessEnv;
...
describe("checkReadiness", () => {
  it("is ready when configuration is valid and the database probe answers true", async () => {
    ...
    await expect(checkReadiness(env, fetchImpl)).resolves.toEqual({ ok: true, checks: { configuration: "ok", database: "ok" } });
  ...
  it("is not ready, and does not call the database, when configuration is missing", async () => {
    ...
    await expect(checkReadiness({} as NodeJS.ProcessEnv, fetchImpl)).resolves.toEqual({ ok: false, checks: { configuration: "failed", database: "failed" } });
